...
int gyro_average_offsets[3] = {GYRO_AVERAGE_OFFSET_X, GYRO_AVERAGE_OFFSET_Y, GYRO_AVERAGE_OFFSET_Z};
...
void compensate_sensor_errors() {
    ...
    // Uses autocalibrated values
    gyro[0] -= gyro_average_offsets[0];
    gyro[1] -= gyro_average_offsets[1];
    gyro[2] -= gyro_average_offsets[2];
}
...
void calibrate_gyro() {
  for(int i = 0; i < 3; i++) {
    gyro_average_offsets[i] = gyro_average[i] / gyro_num_samples;
  }
  
    
  Serial.print(gyro_average_offsets[0]);
  Serial.print(",");
  Serial.println(gyro_average_offsets[1]);
  Serial.print(",");
  Serial.println(gyro_average_offsets[2]);
}
    ...
    if (Serial.read() == '#') // Start of new control message
    {
      ...
      else if (command == 'o') // Set _o_utput mode
      {
        ...
        else if (output_param == 'c') // Go to _c_alibration mode
        {
          ...
          reset_calibration_session_flag = true;
        } else if (output_param == 'k') // _k_eep calibration values
        {
          calibrate_gyro();
        }
